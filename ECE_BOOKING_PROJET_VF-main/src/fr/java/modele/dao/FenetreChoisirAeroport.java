...
public class FenetreChoisirAeroport extends Fenetre {
    /**ATTRIBUTS*/
    /**bouton*/
    ...
    public FenetreChoisirAeroport(ArrayList<Client> listeClient, ArrayList<Employe> listeEmploye, ArrayList<Ville> listeVille, HistoriqueReservation historique, Boolean etat) {
        super(listeClient, listeEmploye, listeVille, historique);

        setTitle("Choisir un aeroport"); //On donne un titre à l'application
    ...
    /** permet de visualiser tous les aeroports d'une ville*/
    public JTable ajoutComposant2() {
        ...
        Object [][] donnee = new Object[getvilleEtudiee().getAeroportAProximite().size()][2];

        for(int i=0;i<getvilleEtudiee().getAeroportAProximite().size();i++){
            donnee[i][0]=getvilleEtudiee().getAeroportAProximite().get(i).getNomAeroport();
            donnee[i][1]=getvilleEtudiee().getAeroportAProximite().get(i).isEstInternational();
        }
        /** CREATION DU MODELE DE DONNEES POUR LA TABLE */
        DefaultTableModel tab = new DefaultTableModel(donnee, new Object[]{"Nom","International"});
